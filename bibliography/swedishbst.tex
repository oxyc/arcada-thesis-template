...
\def\bbland{och}
\def\bbletal{et~al.} % todo dot?
\def\bbleditors{utgivare}
\def\bbleds{utgivare} % todo förkortning
\def\bbleditor{utgivare}
\def\bbled{utgivare} % todo förkortning
\def\bbledby{utgiven av}
\def\bbledition{upplaga}
\def\bbledn{uppl.} % todo shortened version?
\def\bblvolume{volym}
\def\bblvol{vol.}
\def\bblof{av}
\def\bblnumber{nummer}
\def\bblno{nr}
\def\bblin{i} % todo teoksessa -> ?
\def\bblpages{sidor}
\def\bblpp{ss.}
\def\bblpage{sid}
\def\bblp{s.}
\def\bbleidpp{sidor} % todo pages->sivua?
\def\bblchapter{kapitel}
\def\bblchap{kapitel} % todo förkortning
\def\bbltechreport{teknisk rapport} % techreport?
\def\bbltechrep{tek. rap.} % todo rätt?
\def\bblmthesis{Master's thesis} % todo
\def\bblphdthesis{doktorsavhandling}  % todo
\def\bblfirst{första}
\def\bblfirsto{1.}
\def\bblsecond{andra}
\def\bblsecondo{2.}
